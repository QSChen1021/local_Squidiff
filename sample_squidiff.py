...
class sampler:
    ...
    def stochastic_encode(self, model, x, t, model_kwargs):
        """
        ddim reverse sample
        """
        sample = x
        ...
        for i in indices:
            ...
            with torch.no_grad():
                out = self.diffusion.ddim_reverse_sample(
                    model, sample, timestep, model_kwargs=model_kwargs
                )
                sample = out["sample"]
    ...
    def get_diffused_data(self, model, x, t, model_kwargs):
        ...
        for i in indices:
            ...
            with torch.no_grad():
                ...
                out = sample + noise * (
                    i / t
                )  # Simulating diffusion based on time step
                sample = out
    ...
    def interp_with_direction(
        ...
        z_sem_interp_ = (
            z_sem_origin.mean(axis=0) + direction.detach().cpu().numpy() * scale
        )
        if add_noise_term:
            z_sem_interp_ = self.sample_around_point(
                z_sem_interp_, num_samples=z_sem_origin.shape[0]
            )
        ...
        sample_interp = self.sample_fn(
            ...
            model_kwargs={"z_mod": z_sem_interp_},
            noise=None,
        )
        return sample_interp

    def cal_metric(self, x1, x2):
        r2 = r2_score(x1.detach().cpu().numpy().mean(axis=0), x2.X.mean(axis=0))
        pearsonr, _ = scipy.stats.pearsonr(
            x1.detach().cpu().numpy().mean(axis=0), x2.X.mean(axis=0)
        )
        return r2, pearsonr
