...
def Drug_dose_encoder(
    drug_SMILES_list: list, dose_list: list, num_Bits=1024, comb_num=1
):
    """
    ...
    def __init__(self, adata, control_adata=None, use_drug_structure=False, comb_num=1):
        ...
        if self.use_drug_structure:
            if isinstance(control_adata.X, np.ndarray):
                self.control_features = torch.tensor(
                    control_adata.X, dtype=torch.float32
                )
            else:
                self.control_features = torch.tensor(
                    control_adata.X.toarray(), dtype=torch.float32
                )
            ...
            self.encode_drug_doses = torch.tensor(
                self.encode_drug_doses, dtype=torch.float32
            )
        else:
            self.encoded_obs_tensor = adata.obs["Group"].copy().values
...
def prepared_data(
    ...
    _data_dataset = AnnDataDataset(
        train_adata, control_adata, use_drug_structure, comb_num
    )
